...
(* ---------------- Tácticas para las obligaciones de Program Fixpoint leq - *)
...
(*   (* Program Fixpoint leq Modificado Versión 1  *)
Program Fixpoint leq (p : Term * Term) {wf R' p}
  : { b : bool | Is_true b -> Leq (fst p) (snd p) } :=
  match p with
  | (Var m, Var n) => Nat.eqb m n
  | (Join t1 t2, u) =>
      proj1_sig (leq (t1, u)) && proj1_sig (leq (t2, u))
  | (t, Meet u1 u2) =>
      proj1_sig (leq (t, u1)) && proj1_sig (leq (t, u2))
  | (Var m, Join u1 u2) =>
      member m u1 && (proj1_sig (leq (Var m, u1)) || proj1_sig (leq (Var m, u2)))
  | (Meet t1 t2, Var n) =>
      member n (Meet t1 t2) &&
      (proj1_sig (leq (t1, Var n)) || proj1_sig (leq (t2, Var n)))
  | (Meet t1 t2, Join u1 u2) =>
      let b1 := proj1_sig (leq (t1, Join u1 u2)) in
      if b1 then true else
      let b2 := proj1_sig (leq (t2, Join u1 u2)) in
      if b2 then true else
      let b3 := proj1_sig (leq (Meet t1 t2, u1)) in
      if b3 then true else proj1_sig (leq (Meet t1 t2, u2))
  end.
Next Obligation.
 unfold Leq.
  intros env.
  (* Pasar de Is_true (m =? n) a m = n *)
  apply Is_true_eq_true in H.
  apply Nat.eqb_eq in H. subst n.
  (* Ya quedó Var m <= Var m *)
  apply reflexive.
Qed.
*)
...
Ltac variables2 m j env t :=
  match t with
  | (m ?X1 ?X2) =>
      let env' := variables2 m j env X1 in
        variables2 m j env' X2
  | (j ?X1 ?X2) =>
      let env' := variables2 m j env X1 in
        variables2 m j env' X2
  | ?X =>
...
Ltac reflect2 env m j v :=
  match v with
  | (m ?X1 ?X2) =>
      let r1 := reflect2 env m j X1
      with r2 := reflect2 env m j X2 in
        constr:(Meet r1 r2)
  | (j ?X1 ?X2) =>
      let r1 := reflect2 env m j X1
      with r2 := reflect2 env m j X2 in
...
(* (* reflect_eq sin corregir *)
Ltac reflect_eq A l m j :=
  match goal with
  | [ |- (?X1 = ?X2 :> A) ] =>
    let vs := variables2 m j (nil (A := A)) X1 in
    let vs' := variables2 m j vs X2 in
    let env := constr:(fun n => nth n vs' X1) in
    let term1 := reflect2 vs' m j X1 
    with term2 := reflect2 vs' m j X2 in
    (
...
Ltac lattice_solve_inequality A l o m j :=
  ...
  | [ |- (o ?X1 ?X2) ] =>
    ...
    (
(*idtac term1; idtac term2;*)
      change (o (eval term1 env) (eval term2 env));
     apply less_correct;
      try (subst; reflexivity);
     (* instantiate; *) vm_compute;
(*     compute;*)
...
Lemma less_correct2 :
  forall (X1 X2 : A) (term1 term2 : Term) (env : nat -> A),
    X1 = eval term1 env ->
    X2 = eval term2 env ->
    le (eval term1 env) (eval term2 env) ->
    le X1 X2.
Proof.
  intros X1 X2 t1 t2 env -> -> H.
  exact H.
Qed.
...
Lemma median : forall (x y z:A),
  ...
  Proof.
    ...
    match goal with
    | [ |- (le ?X1 ?X2) ] =>
        let vs := variables2 meet join (nil (A:=A)) X1 in
        let vs' := variables2 meet join vs X2 in
        let env := constr:(fun n => nth n vs' X1) in
        let  term1 := reflect2 vs' meet join X1
        with term2 := reflect2 vs' meet join X2 in
        (
          (* change (le (eval (A:=A) (l:=l) term1 env) (eval (A:=A) (l:=l) term2 env)) *)
          change (le (eval term1 env) (eval term2 env)) 
        )
    end.
    apply less_correct.
    Time vm_compute.
...
Proof.
  ...
  | [ |- le ?X1 ?X2 ] =>
      ...
      let vs' := variables2 meet join vs X2 in
      let env := constr:(fun n => nth n vs' X1) in
      let term1 := reflect2 vs' meet join X1 in
      let term2 := reflect2 vs' meet join X2 in
      apply (less_correct2 X1 X2 term1 term2 env);
