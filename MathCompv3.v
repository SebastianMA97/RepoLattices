...
Structure ordtype (T : Type) := Ordtype { tord :> T -> T -> Prop; OT : order tord }.

Structure jointype {T : Type} (tord : ordtype T) := Jointype 
  { joint :> T -> T -> T;
     JT : (forall z : T , forall x y : T,  (tord x z /\ tord y z) <-> tord ( joint x y) z) }.

(*Variables (K : Type ) (ord_K : ordtype K) (j : jointype ord_K).

Check JT.*)

Structure meettype {T : Type} (tord : ordtype T) := Meettype 
  { meett :> T -> T -> T;
     _ : (forall z : T , forall x y : T,  (tord z x /\ tord z x) <-> tord z ( meett x y)) }.
...
split.
  ...
  have cotainf : ord_L a (join_L a b) /\ ord_L b (join_L a b).
    move: (JT ord_L join_L (join_L a b) a b ); move=> /proj2.
    move: (OT L ord_L) ; move=> /proj1; unfold reflexive; move=> ref.
    by move: (ref (join_L a b)); move=> H; move=> H2; move: H; move=> /H2.
